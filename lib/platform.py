...
class PlatformRasp(PlatformBase):
    ...
    def get_current_connections():
        # Bookworm and newer Raspberry Pi OS releases use NetworkManager by default.
        # Prefer nmcli and only fall back to iwconfig on older images.
        try:
            ...
            if 'Hotspot' in hotspot.stdout.splitlines():
    ...
    def get_wifi_networks():
        # Prefer nmcli on newer Raspberry Pi OS releases where NetworkManager is standard.
        try:
            result = subprocess.run(
                ['nmcli', '-t', '-f', 'SSID,SIGNAL,BSSID,FREQ,ACTIVE', 'dev', 'wifi', 'list', '--rescan', 'yes'],
                capture_output=True,
                text=True,
                check=True
            )
            ...
            for line in result.stdout.splitlines():
                ...
                ssid, signal, bssid, freq, _active = parts[:5]
                ssid = ssid.strip()
                if not ssid:
                    continue
                ...
                signal_dbm = int((signal_strength / 100.0) * 40 - 90)
                wifi_data = {
                    'Address': bssid.strip(),
                    'ESSID': ssid,
                ...
                if signal_strength > wifi_dict[ssid]['Signal Strength']:
        ...
        try:
            ...
            logger.warning("No Wi-Fi scanning utility found")
            return []
